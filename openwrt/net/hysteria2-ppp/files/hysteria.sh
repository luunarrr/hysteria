...
proto_hysteria_setup() {
	...
	*)
	...
	if touch "$metafile" 2>/dev/null && chmod 644 "$metafile" 2>/dev/null; then
		{
			...
			[ "$use_mp" = 1 ] && echo "bundle_mtu=$((mtu - 6))"
	...
	local pty_cmd="HYSTERIA_PPP_STATUS='$statusfile' HYSTERIA_PPP_STATE='$(hysteria_slot_client "$config" 1)' /usr/bin/hysteria2-ppp client -c '$cfgfile'$pty_err"
