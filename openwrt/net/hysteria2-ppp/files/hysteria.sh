...
# hysteria_gate_file and hysteria_sup_pid moved to hysteria2-ppp-common, which is
# sourced above: the status reader needs both and has no business sourcing a
# netifd protocol handler to reach them.
...
hysteria_write_config() {
	...
	*)
	...
	# The configured MRU ceiling. In nospawn mode the client is pppd's pty child,
	# so pppd fixed its MRU before the client existed and nothing the client
	# measures could change it in this session -- telling it the number instead
	# means it skips a path measurement that would cost up to five seconds per
	# dial to learn something it cannot apply here.
	#
	# The configured figure rather than the one pppd was actually started with,
	# which may be lower after a previous session measured the path. What the
	# client offers here bounds what the server negotiates, so offering the
	# learned figure would make every session confirm the previous one's number
	# and a path that widened again could never be discovered. Offering the
	# ceiling each time keeps the measurement honest in both directions; see
	# hysteria_learned_mru.
	echo "  mtu: $mtu" >> "$file"
...
# Counted as they arrive rather than derived from the list later, because the
# list is emptied when multilink is switched off -- and an interface with extra
# servers configured but disabled is then indistinguishable at runtime from one
# with none. "Why is only one server listed" would have no answer anywhere.
hysteria_extra_count=0
...
hysteria_collect_extra_server() {
	...
	[ -n "$1" ] || return 0
	hysteria_extra_servers="$hysteria_extra_servers $1"
	hysteria_extra_count=$((hysteria_extra_count + 1))
}
...
hysteria_links_stop() {
	...
	while [ "$n" -le "$HYSTERIA_MAX_LINKS" ]; do
		...
		rm -f "$(hysteria_slot_yaml "$config" "$n")" \
			...
			"$(hysteria_slot_state "$config" "$n")" \
			"$(hysteria_slot_client "$config" "$n")" \
			"$(hysteria_slot_bytes "$config" "$n")" \
			"$(hysteria_slot_lasterr "$config" "$n")" \
			"$(hysteria_slot_pppdlog "$config" "$n")" \
	...
	# The status reader keys off this file to decide an interface exists at all,
	# so it goes with everything else it describes. Left behind by a crash it
	# would advertise a bundle whose slots have all been swept, which reads as
	# every server having failed at once rather than as nothing running.
	rm -f "$(hysteria_meta_file "$config")"
...
proto_hysteria_setup() {
	...
	*)
	...
	# 21 for path-MTU discovery's convergence margin, less the 37 quic-go reserves
	# per packet and the 4-octet PPP header, which leaves 1390. Left at 1500 a
	# default install comes up and then silently drops every full-size packet.
	#
	# 37 and not the 28 octets a QUIC datagram packet actually spends on headers.
	# quic-go refuses a datagram against its own estimate, which assumes the
	# maximum connection ID length rather than the one in use, and that estimate is
	# what a frame has to fit; see quicDatagramReserve in extras/pppbridge/mtu.go.
	# This read 1399 while that was modelled as 28, which was nine octets of MTU
	# the transport would not take.
	#
	...
	# What pppd is actually started with. The number above is a guess -- 1390 is
	# what a 1500-octet Ethernet path leaves after QUIC, and nothing here has
	# measured the path to the servers. A previous session may have: the client
	# records the MTU it negotiated whenever it came out below this, and that
	# measurement is the only way the figure can reach a pppd which is this
	# process's parent and fixed its MRU before the client existed.
	#
	# $mtu itself stays as configured, and is what goes into the client YAML. That
	# is deliberate and is what stops this ratcheting: the client offers the
	# configured ceiling every time, so a path that widens is measured as wider and
	# the next bring-up recovers. Feeding the learned figure back as the offer
	# would make each session confirm the last one's number and never climb.
	local link_mru
	link_mru=$(hysteria_learned_mru "$config" "$mtu")
	[ -n "$link_mru" ] || link_mru="$mtu"
	if [ "$link_mru" != "$mtu" ]; then
		logger -t hysteria -p daemon.notice \
			"$config: starting pppd at the measured link MRU $link_mru, not the configured $mtu"
	fi
	...
	if [ "$use_mp" = 1 ]; then
		...
		fi
		{
			...
			echo "set HYSTERIA_BUNDLE_MTU=$((link_mru - 6))"
		...
		# mru goes on the command line and not in the file above, and it is the
		# one option here that has to. It carries OPT_PRIO, and process_option
		# refuses a value whose priority is below the stored one -- a file is
		# OPRIO_CFGFILE (1) against ppp.sh's own "mru $mtu" at OPRIO_CMDLINE (2),
		# so a file can never override it and does not say why. Position is what
		# decides between two values of equal priority, and ppp_generic_setup
		# appends "$@" after everything it generates, so this one is later and
		# wins.
		#
		# Left in the file it silently kept the configured ceiling: the bundle
		# MTU dropped to the measured figure while the MRU advertised to the LNS
		# stayed where it was, so the far end went on sending frames this side had
		# just measured the path as unable to carry. That is the exact failure the
		# measurement exists to prevent, in the one direction nothing here would
		# have noticed.
		#
		# mtu and mrru stay in the file. mtu is not OPT_PRIO, so its position
		# there -- after ppp.sh's -- is enough, and ppp.sh never sets mrru at all.
		set -- mru "$link_mru" file "$holderopts"
	else
		# The same figure, reaching pppd the same way, for an interface with one
		# server. This is where the measurement used to stop: link_mru was
		# computed and logged for every interface but only ever applied inside the
		# branch above, so a single-server link went on being started at the
		# configured ceiling while the log said otherwise. Nothing else passes
		# mtu or mru here, so ppp.sh's -- built from the configured value -- was
		# the only setting there was.
		#
		# No MLPPPOverhead: without multilink there is no bundle sitting below the
		# link, and no fragment header to leave room for. What the link carries is
		# what the interface gets.
		set -- mtu "$link_mru" mru "$link_mru"
	fi

	# What a status reader needs to describe this interface, and nothing else.
	#
	# Everything here is already in the .link env file, which is mode 600 because
	# the PPP password is in it too. A reader answering an unprivileged query
	# should not have to open that file at all -- not "should open it carefully",
	# should not have to. This one is mode 644 and holds no secret, so the
	# question of what it might leak does not arise.
	#
	# Written for single-server interfaces as well. "Which server is connected"
	# has one answer there rather than none, and an interface that vanishes from
	# the status page as soon as multilink is turned off is a worse answer than
	# a short list.
	local metafile
	metafile=$(hysteria_meta_file "$config")
	rm -f "$metafile"
	if touch "$metafile" 2>/dev/null && chmod 644 "$metafile" 2>/dev/null; then
		{
			echo "ifname=$pppname"
			echo "multilink=$use_mp"
			echo "slots=$slot_count"
			# link_mru rather than the configured mtu: this file answers "what is
			# this interface", and after a session has measured the path those are
			# different numbers. Reporting the ceiling here showed an MTU no packet
			# on the interface was ever built to, which is worse than showing
			# nothing -- it is the figure an operator would check against when the
			# link is dropping large packets, and it would agree with them.
			echo "mtu=$link_mru"
			[ "$use_mp" = 1 ] || echo "servers_ignored=$hysteria_extra_count"
			[ "$use_mp" = 1 ] && echo "bundle_mtu=$((link_mru - 6))"
			[ -n "$bundle" ] && echo "bundle=$bundle"
			[ -n "$endpoint" ] && echo "endpoint=$endpoint"
		} >> "$metafile"
	else
		# Not fatal, and deliberately so. This file exists to be read by a status
		# page; an interface must never fail to come up because one could not be
		# written.
		echo "hysteria: cannot write $metafile, link status will be unavailable" >&2
	fi
	...
	# HYSTERIA_PPP_STATE is set here as well as on the bundle path, because a
	# single-server interface has the same question asked of it -- "is the server
	# connected" -- and the same answer available only from the client. Slot 1 is
	# the only slot there is when this line is the one that runs.
	local pty_cmd="HYSTERIA_PPP_STATUS='$statusfile' HYSTERIA_PPP_STATE='$(hysteria_slot_client "$config" 1)' HYSTERIA_PPP_MTU='$(hysteria_slot_mtu "$config" 1)' HYSTERIA_PPP_BYTES='$(hysteria_slot_bytes "$config" 1)' /usr/bin/hysteria2-ppp client -c '$cfgfile'$pty_err"
	...
	if [ "$use_mp" = 1 ]; then
		...
		fi
		{
			...
			hysteria_env_line HY_LINK_MRU "$link_mru"
			hysteria_env_line HY_BUNDLE_MTU "$((link_mru - 6))"
