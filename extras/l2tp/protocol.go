...
func BuildICCN(info *ProxyInfo) []byte {
	...
	if len(info.InitialReceivedCONFREQ) > 4 {
		buf = append(buf, EncodeBytesAVP(AVPInitialReceivedLCPCONFREQ, info.InitialReceivedCONFREQ[4:])...)
	}
	if len(info.LastSentCONFREQ) > 4 {
		buf = append(buf, EncodeBytesAVP(AVPLastSentLCPCONFREQ, info.LastSentCONFREQ[4:])...)
	}
	if len(info.LastReceivedCONFREQ) > 4 {
		buf = append(buf, EncodeBytesAVP(AVPLastReceivedLCPCONFREQ, info.LastReceivedCONFREQ[4:])...)
	}
