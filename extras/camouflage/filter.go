...
func (f *Filter) processPacket(data []byte, src net.Addr) bool {
	...
	if f.config.RateLimiter != nil && f.config.RateLimiter.IsRateLimited(rlKey) {
		f.relay(data, src, "rate limited, not re-checked")
		return false
	}
	...
	if err != nil {
		...
		f.relay(data, src, "unparseable QUIC header")
		return false
	}
	...
	if !isInitial {
		if dcid == nil {
			...
			f.recordFailure(rlKey)
			f.relay(data, src, "short header from an unauthenticated source")
			return false
		}
		...
		f.relay(data, src, "non-Initial long header from an unauthenticated source")
		return false
	}
	...
	if !result.Matched() {
		f.recordFailure(rlKey)
		f.relay(data, src, "no configured secret matches the token")
		return false
	}
	...
	if !result.ServerIDMatch {
		...
		f.recordFailure(rlKey)
		f.relay(data, src, "token minted against a different address")
		return false
	}
	...
	if f.isReplay(dcidKey, srcKey) {
		...
		f.recordFailure(rlKey)
		f.relay(data, src, "token already used by a different source")
		return false
	}
...
func (f *Filter) relay(data []byte, src net.Addr, reason string) {
