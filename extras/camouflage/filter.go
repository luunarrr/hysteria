...
type FilterConfig struct {
	...
	// ServerIPs are the addresses clients are expected to have minted their
	// tokens against. Empty skips the server_id check entirely.
	//
	// Naming any address at all turns the check on; naming the right ones is
	// then only half the job, because a server on the wildcard also answers at
	// addresses the operator never wrote down. The address a packet actually
	// arrived on is accepted alongside these -- see expectedServerIPs.
	ServerIPs      []net.IP
...
//
// oob is the control message this packet arrived with, or nil. It says which of
// this host's addresses the packet was sent to, which both the decoy reply path
// and the server_id check need -- see expectedServerIPs and replySourceOOB.
func (f *Filter) processPacket(data []byte, src net.Addr, oob []byte) bool {
	...
	result := VerifyDCIDAny(f.config.Secrets, f.expectedServerIPs(oob), dcid, f.config.TimeBucketSize)
	...
	// probe. Reaching here means the token names neither a configured address
	// nor the one this packet was sent to, so the remaining explanations are a
	// client that mints against an address this server never sees -- a NAT or a
	// load balancer in front of it, which serverAddr exists to name -- or a
	// token genuinely replayed from another server.
	if !result.ServerIDMatch {
		if f.config.AlertFunc != nil {
			f.config.AlertFunc(src, "valid credential, but the token was minted against an address this server neither holds nor has configured -- name that address in serverAddr if a NAT or load balancer sits in front (otherwise: cross-server replay)", result.Label)
		}
		f.recordFailure(rlKey)
...
// expectedServerIPs is the set of addresses a client's token is allowed to name.
//
// The configured addresses plus the one this packet was actually sent to. That
// second term is what makes "listen on all interfaces" mean it: a wildcard
// socket answers at every address the host has, an operator can only write down
// the ones they thought of, and the client mints against whichever one it
// dialled. Without it a server holding two addresses authenticates clients on
// the first and hands every client on the second to the decoy -- which looks,
// from the client, like a server that is simply not there, and from the server
// like nothing at all.
//
// The kernel is the authority for that second term and it is not spoofable: it
// is the destination the packet really carried, so the only addresses it can
// ever produce are ones this host already answers at.
//
// Configured addresses are still honoured, and still needed, for the case the
// arrival address cannot speak to -- a NAT or load balancer in front, where the
// address the client dialled is not the address the packet arrives on.
//
// Returning nil when nothing is configured keeps "serverAddr unset" meaning the
// check is off. Enabling it here on the strength of an address quic-go happens
// to hand us would switch on a rejection the operator never asked for, and its
// failure mode is silence.
func (f *Filter) expectedServerIPs(oob []byte) []net.IP {
	if len(f.config.ServerIPs) == 0 {
		return nil
	}
	ip, _, _, ok := localDstFromOOB(oob)
	if !ok {
		return f.config.ServerIPs
	}
	// Full slice expression: append must copy rather than write into an array
	// shared with every other packet being processed.
	return append(f.config.ServerIPs[:len(f.config.ServerIPs):len(f.config.ServerIPs)], ip)
}
